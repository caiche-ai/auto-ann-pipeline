...
from ..pipeline.grounding_dino.prompt import (
...
class ReviewTaskBuildItem(StrictModel):
    detection_id: str
    detection_ids: List[str] = Field(..., min_items=1)
    task_id: str
    task_version: int = Field(..., ge=1)
    asset_id: str
    box_xyxy: List[float]
    boxes_xyxy: List[List[float]] = Field(..., min_items=1)
    created: bool
...
class CreateMaskCandidateRequest(StrictModel):
    ...
    boxes_xyxy: Optional[List[List[float]]] = Field(
        default=None,
        min_items=1,
        max_items=128,
    )
    ...
    @validator("box_xyxy")
    def single_box_is_valid(cls, value):
        return _validate_box(value) if value is not None else value

    @validator("boxes_xyxy")
    def boxes_are_valid(cls, value):
        return (
            [_validate_box(box) for box in value]
            if value is not None
            else value
        )

    @root_validator(skip_on_failure=True)
    def exactly_one_box_form_is_required(cls, values):
        single = values.get("box_xyxy")
        multiple = values.get("boxes_xyxy")
        if (single is None) == (multiple is None):
            raise ValueError(
                "provide exactly one of box_xyxy or boxes_xyxy"
            )
        boxes = multiple or [single]
        detection_ids = values.get("detection_ids") or []
        if detection_ids and len(detection_ids) != len(boxes):
            raise ValueError(
                "detection_ids must match the number of boxes"
            )
