...
from ..api.errors import InvalidStateTransitionError, ValidationServiceError
from ..api.schemas import JobStatus
from ..storage.repository import AnnotationStore, utc_now
...
def build_detection_review_tasks(
    ...
    *,
    ...
    """Create one sample-level Task per image for selected detections."""
    ...
    selected_ids = [
        detection_id
        for detection_id in by_id
        if detection_id in requested_set
    ]
    selected_by_asset: dict[str, list[dict[str, Any]]] = {}
    for detection_id in selected_ids:
        detection = by_id[detection_id]
        selected_by_asset.setdefault(
            detection["asset_id"],
            [],
        ).append(detection)
    ...
    for asset_id, selected in selected_by_asset.items():
        asset = store.get_asset(asset_id)
        detection_ids_for_task = [
            detection["detection_id"] for detection in selected
        ]
        boxes = [detection["box_xyxy"] for detection in selected]
        first = selected[0]
        ...
        entities = list(
            dict.fromkeys(
                detection["entity"].strip() or "目标对象"
                for detection in selected
            )
        )
        target_object = "、".join(entities)
        task = store.create_task(
            job_id=job_id,
            asset_id=asset_id,
            category=category,
            annotation={
                ...
                "visual_anchor": [
                    ...
                    for detection in selected[:10]
                ],
                "mask_granularity": "全部人工选中目标的联合区域",
                "risk_semantics": None,
                "shapes": [],
                "prompts": [],
            },
            provenance={
                "pipeline_version": job["pipeline_version"],
                "source_detection_id": detection_ids_for_task[0],
                "source_detection_ids": detection_ids_for_task,
                ...
                "review_task_builder_version": (
                    "sample-level-detection-task-v2"
                ),
            },
            warnings=[
        ...
        items.append(
            {
                "detection_id": detection_ids_for_task[0],
                "detection_ids": detection_ids_for_task,
