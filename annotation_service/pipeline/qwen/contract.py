...
class QwenVisualFacts(StrictModel):
    ...
    def text_is_trimmed(cls, value):
        if not isinstance(value, str):
            return value
        normalized = value.strip()
        # The model may explicitly return an empty optional risk semantic when
        # the pixels do not support a risk claim.  Treat that exactly like a
        # missing value; ground_prompt_set then safely falls back to a visible
        # fact instead of turning a valid response into a failed operation.
...
def ground_prompt_set(*, facts: QwenVisualFacts) -> QwenPromptSet:
    """Deterministically build 3+2+1 prompts from validated facts."""

    target = facts.target_object[:60].rstrip("，。； ")
    anchor = "、".join(facts.visual_anchor[:2])[:40].rstrip("，。； ")
    visible_fact = facts.visible_facts[0][:40].rstrip("，。； ")
    granularity = facts.mask_granularity[:40].rstrip("，。； ")
    risk = (facts.risk_semantics or visible_fact)[:40].rstrip("，。； ")
    grounded = [
        AnnotationPrompt(
            prompt_id="visual-1",
        ...
        AnnotationPrompt(
            prompt_id="visual-2",
            type=PromptType.VISUAL,
            text=f"标出{target}；视觉锚点：{anchor}。",
        ),
        AnnotationPrompt(
            prompt_id="visual-3",
            type=PromptType.VISUAL,
            text=(
                f"提取{granularity}对应的{target}；"
                f"可见事实：{visible_fact}。"
            ),
        ),
        AnnotationPrompt(
            ...
            text=f"从安全标注角度分割{target}；风险语义：{risk}。",
        ),
        AnnotationPrompt(
            prompt_id="risk-2",
            type=PromptType.RISK,
            text=f"标出{target}并核验{risk}；保持{granularity}。",
        ),
        AnnotationPrompt(
            prompt_id="agent-1",
            type=PromptType.AGENT,
            text=f"请定位并完整分割{target}，保留{granularity}。",
        ),
    ]
    try:
        ...
        raise QwenContractError(
            "grounded prompts exceed the annotation contract"
        ) from exc
