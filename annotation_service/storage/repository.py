...
class AnnotationStore:
    ...
    def initialize(self) -> None:
        with self._lock:
            ...
            try:
                ...
                self._migrate_legacy_review_submissions()
                for path in (
                    self.images_root,
                    self.masks_root,
                    self.overlays_root,
    ...
    def _migrate_legacy_review_submissions(self) -> None:
        """Move the pre-refactor review/submissions tree into submissions."""

        legacy_root = self.root / "review" / "submissions"
        if not legacy_root.is_dir():
            return
        if not self.submissions_root.exists():
            os.replace(legacy_root, self.submissions_root)
        else:
            for source in sorted(legacy_root.rglob("*")):
                if not source.is_file():
                    continue
                destination = self.submissions_root / source.relative_to(legacy_root)
                destination.parent.mkdir(parents=True, exist_ok=True)
                if destination.exists():
                    if source.read_bytes() != destination.read_bytes():
                        raise StorageError(
                            "legacy and current submission snapshots conflict"
                        )
                    source.unlink()
                else:
    ...
    def _task_payload(
        ...
        source_detection_ids = provenance.get(
            "source_detection_ids",
            [],
        )
        if not source_detection_ids and row["source_detection_id"]:
            source_detection_ids = [row["source_detection_id"]]
        selected_detection_ids = set(source_detection_ids)
        ...
        return {
            ...
            "detections": [
                ...
                for item in detections
                if (
                    not selected_detection_ids
                    or item["detection_id"] in selected_detection_ids
                )
            ],
        ...
        *,
        ...
        with self._lock, self._connect() as connection:
            ...
            row = self._row_or_not_found(
                ...
                """
                SELECT
                    t.*, a.group_id, a.width, a.height, a.image_path,
                    a.media_type, a.sha256 AS image_sha256
                FROM annotation_tasks t
                JOIN assets a ON a.asset_id = t.asset_id
                WHERE t.task_id = ?
                """,
    ...
    def list_historical_review_submissions(self) -> list[dict[str, Any]]:
        """Return every version created by an annotator submit action."""
        ...
        with self._connect() as connection:
            rows = connection.execute(
                ...
                SELECT
                    ...
                    t.primary_result, t.provenance_json, t.warnings_json,
                    tv.version AS task_version,
                    ...
                    a.group_id, a.width, a.height, a.image_path,
                    a.media_type, a.sha256 AS image_sha256
                FROM task_versions tv
                JOIN annotation_tasks t ON t.task_id = tv.task_id
                JOIN assets a ON a.asset_id = t.asset_id
                WHERE tv.change_kind = 'submit'
                ORDER BY t.task_id, tv.version
                """
            ).fetchall()
        return [
            self._review_submission_payload(
                ...
                annotation=_json_loads(row["submitted_annotation_json"], {}),
                annotator_id=row["submitted_by"],
                primary_result=row["primary_result"],
                comment=row["submission_comment"],
    ...
    def create_mask_candidate_operation(
        ...
        *,
        ...
        if expected_version < 1:
            raise ValueError("expected_version must be positive")
        if (box_xyxy is None) == (boxes_xyxy is None):
            raise ValueError(
                "provide exactly one of box_xyxy or boxes_xyxy"
            )
        raw_boxes = boxes_xyxy or [box_xyxy]
        boxes: list[list[float]] = []
        for raw_box in raw_boxes:
            if raw_box is None or len(raw_box) != 4:
                raise ValueError(
                    "each box_xyxy must contain four coordinates"
                )
            ...
            boxes.append(box)
        normalized_detection_ids = [
            str(value).strip() for value in (detection_ids or [])
        ]
        if normalized_detection_ids and (
            len(normalized_detection_ids) != len(boxes)
            or any(not value for value in normalized_detection_ids)
            or len(normalized_detection_ids)
            != len(set(normalized_detection_ids))
        ):
            raise ValueError(
                "detection_ids must uniquely match boxes_xyxy"
            )
        request_payload = {
            "boxes_xyxy": boxes,
            "detection_ids": normalized_detection_ids,
        }
        if len(boxes) == 1:
            request_payload["box_xyxy"] = boxes[0]
        request_json = canonical_json(request_payload)
        operation_id = _new_id("op")
        created_at = utc_now()
        with self._lock, self._connect() as connection:
            ...
            if any(
                box[2] > task["width"] or box[3] > task["height"]
                for box in boxes
            ):
                connection.execute("ROLLBACK")
                raise ValidationServiceError(
